...
class Config:
    ...
    def _read_geometry_parameters(self):
        ...
        if self.config.get("sampling"):
            ...
            # opts: tblite | xtb | ani
            self.sampling_calculator = self.config["sampling"].get(
                "calculator", "tblite"
            )
            self.sampling_device = self.config["sampling"].get("device", "cpu")
            self.sampling_mace_model_path = self.config["sampling"].get(
                "mace_model_path", None
            )

            # sampling.md:
            if self.config["sampling"].get("md"):
                self.sampling_md_interval = self.config["sampling"]["md"].get(
                    "sampling_interval", 20
                )
                self.sampling_md_temperature = self.config["sampling"]["md"].get(
                    "temperature", 500
                )
                self.sampling_md_timestep = self.config["sampling"]["md"].get(
                    "timestep", 0.5
                )
                self.sampling_md_steps = self.config["sampling"]["md"].get(
                    "md_steps", 1000
                )
            ...
            if self.config["sampling"].get("rattle"):
                self.sampling_rattle_method = self.config["sampling"]["rattle"].get(
                    "method", "mc"
                )
                self.sampling_rattle_nstructures = self.config["sampling"][
                    "rattle"
                ].get("rattle_structures", 10)
                self.sampling_rattle_std = self.config["sampling"]["rattle"].get(
                    "rattle_std", 0.12
                )
                ...
                if self.config["sampling"]["rattle"].get("optimize"):
                    ...
                    self.sampling_optimize_rattled_fmax = self.config["sampling"][
                        "rattle"
                    ]["optimize"].get("fmax", 1.0)
                    self.sampling_optimize_rattled_maxStep = self.config["sampling"][
                        "rattle"
                    ]["optimize"].get("max_step", 0.1)
    ...
    def _read_calculator_parameters(self):
        ...
        if self.calculator is not None:
            ...
            self.calculator_writeInput = self.config["calculator"].get(
        ...
        if self.calculator == "aims":
            default_input_params_aims = {
                "species_dir": "/home/username/opt/fhi-aims/species_defaults/defaults_2020/light",
                ...
                "k_grid": [1, 1, 1],
                # "DFPT": "dielectric",
                # DFPT_centralised : True,   # In new AIMS versions is True by default
                # DFPT_width : 0.01,
                "command": "mpirun -n 8 /home/username/bin/aims.x",
                "output": ["dipole"],
            }

            self.input_params = self.config["calculator"].get(
                "aims", default_input_params_aims
            )
